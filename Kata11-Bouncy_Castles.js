...
const sphereVolume = function (radius) { // Fucntion to calculate the volume of sphere.
  return (4/3)*PI*radius*radius*radius;
}

console.log(4186 < sphereVolume(10) && sphereVolume(10) < 4189);

const coneVolume = function (radius, height) { //Fucntion to calculate the volume of cone. 
  return PI*radius*radius*height/3; 
}

console.log(45 < coneVolume(3, 5) && coneVolume(3, 5) < 49);

const prismVolume = function (height, width, depth) { //Fucntion to calculate the volume of prism.
  return height*width*depth;
}

console.log(prismVolume(3, 4, 5) === 60);

const totalVolume = function (solids) { // Functio  totalVolume() will receive an array containing the different shapes that make up a single attraction. The totalVolume function should use the previous three functions to calculate the total volume of an attraction.
  let volume = 0; //Declaring variable to return total volume. 
  for(solid of solids) { // Running a loop through each element of the solids array. 
    if(solid.type === 'sphere') { // If type is any kind of sphere it will call on the sphere volume function and use the radius corresponding with the solid object radius. Since the object name is different for large vs small sphere, we can use the same if condition.
  ...
  } return volume; // Returning total volume to the totalVolume function. 
}
